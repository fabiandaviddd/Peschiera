...
// 81 Orte tragen die Koordinaten bereits in der Datei — die Seite holt nur den Rest
ok('Zeigt den echten Stand', /^\d+ von 101$/.test((await page.textContent('#count')).trim()),
   await page.textContent('#count'));
const schon = parseInt(await page.textContent('#count'));
ok('Erkennt bereits gesetzte Koordinaten', schon === 78, String(schon));
...
ok('Zurücksetzen fällt auf den Dateistand zurück',
   (await page.textContent('#count')).trim()==='78 von 101', await page.textContent('#count'));
